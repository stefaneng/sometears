...
We simulate from the DAG by specifying the adjacency matrix $B$ and assume that the errors are normally distributed with mean 0 and variance 1 (See Peters and Bühlmann 2014 for more information).
...
d <- ncol(B)
X <- sim_linear_sem(B, n = 100, Sigma = 1 * diag(ncol(B)))
```
...
```{r}
dagma_W <- dagma_fit_linear(X, trace = T,
  epoch = 10,
  s = c(5, 3, 2, 1, 1),
  lr = 0.5,
